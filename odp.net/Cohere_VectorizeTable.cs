...
namespace ConsoleApp9
{
    class Program
    {
        static string embedding_model = "embed-english-v3.0";
        ...
        static void Vectorize_Batch(int embedSize, int commitSize)
        {
            ...
            while (reader.Read())
            {
                ...
                {
                    //get the embeddings
                    embeddings = GetBatchEmbeddingsUsingCoherence(data, embedding_model);
        ...
        /// <returns></returns>
        static string[] GetBatchEmbeddingsUsingCoherence(string[] texts, string embeddingModel)
        {
            ...
            using (HttpClient client = new HttpClient())
            {
                var request = new HttpRequestMessage
                {
                    ...
                    Content = new StringContent($@"{{""texts"":[{batched_text}],""model"":""{embeddingModel}"",""input_type"":""search_query""}}")
                    {
                        Headers =
                    {
                ...
                Console.WriteLine($"Requesting embeddings from https://api.cohere.ai/v1/embed for {texts.Length} texts");
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    {
                        Console.WriteLine($"Parsing response for {subValue.GetArrayLength()} embedding."); 

                        for (int i = 0; i < subValue.GetArrayLength(); i++)
                            embedding[i] = subValue[i].ToString();
                    }
                    else
                    {
        ...
        static void UpdateBatchEmbeddings(int[] ids, string[] embeddings)
        {
            ...
            //oracleCommand1.CommandText = "update test_data set info = :1 where id = :2";
