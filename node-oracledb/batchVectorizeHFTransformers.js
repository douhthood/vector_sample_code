 ...
 *
 * NAME
 *   batchVectorizeHFTransformers.js
 *
 * DESCRIPTION
 *   Add or update the vectors for all data values in a table using Hugging
 *   Face Transformers embedding models with batch vectorization
  ...
  try {
    ...
    while (count < countOfId) {
      ...
      for (let x = 0; x < embedIterations; x++) {
        ...
        const response = await extractor(data.slice(lowerIdx, upperIdx),
          { pooling: 'mean', normalize: true });

        //Get the vector length per data
        const vectorLength = response.data.length / embedSize;
        ...
        for (let y = 0; y < embedSize; y++) {
          ...
          binds.push([response.data.slice(y * vectorLength, (y * vectorLength) + vectorLength), ids[id]]);
          id++;
        }
        lowerIdx = upperIdx;
...
// Uncomment the block below to update the vectors to the table & commit
      ...
      for (let c = 0; c < numCommits; c++) {
        ...
        console.log('Committed ' + subList.length + ' updates');
      }

      count += rowsReturned;
      curMinId = curMaxId;
      curMaxId += rowsReturned;
