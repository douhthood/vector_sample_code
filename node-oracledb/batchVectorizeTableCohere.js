 ...
 *   models in Cohere using batch vectorization
 *   http://www.cohere.com
 *
 *****************************************************************************/
...
async function vectorize() {
  // Cohere's embed API supports 96 texts per call
  const MAX_COHERE_EMBED_SIZE = 96;
  ...
  //   Make embedSize in the range [10, 96]
  //   Make embedSize a factor of fetchSize, if possible
  let embedSize = 50;
  ...
  //   Make commitSize in the range [10, 96]
  ...
  if (embedSize > MAX_COHERE_EMBED_SIZE) {
    embedSize = MAX_COHERE_EMBED_SIZE;
    console.log('embedSize =' + embedSize);
    console.log('embedSize changed as Cohere supports max of 96 texts per call');
  }
  ...
  try {
    ...
    while (count < countOfId) {
      ...
      for (let x = 0; x < embedIterations; x++) {
        ...
        // Cohere's embed API supports 96 texts per call.
        // This is handled in the python wrapper to support any number of texts
        // ,but Node.js Cohere module only supports a max of 96 texts per call.
        const response = await co.embed({
          texts: data.slice(lowerIdx, upperIdx),
          model: embeddingModel,
          inputType: 'search_query'
        });

        console.log('Created ' + response.embeddings.length + ' vectors');
        ...
        for (let y = 0; y < embedSize; y++) {
          ...
          const float32VecArray = new Float32Array(response.embeddings[y]);
...
*/
      ...
      console.log('  curMinId = ' + curMinId + ', curMaxId = ' + curMaxId);
